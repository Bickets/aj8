...
public final class MobSynchronizationEventEncoder extends EventEncoder<MobSynchronizationEvent>
{
	...
	public GamePacket encode( MobSynchronizationEvent event )
	{
		...
		event.getSegments().forEach( segment -> {
			...
			if( type == SegmentType.REMOVE_CHARACTER ) {
				...
				putAddMobUpdate( ( AddCharacterSegment )segment, event, builder );
	 ...
	 */
	private void putAddMobUpdate( AddCharacterSegment seg, MobSynchronizationEvent event, GamePacketBuilder builder )
	{
		boolean updateRequired = seg.getBlockSet().size() > 0;
		Position mobPosition = event.getPosition();
	 ...
	 * @param builder The builder.
	 */
	private void putMovementUpdate( SynchronizationSegment segment, MobSynchronizationEvent event, GamePacketBuilder builder )
	{
