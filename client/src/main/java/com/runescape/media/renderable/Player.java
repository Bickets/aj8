...
public class Player extends GameCharacter {
	...
	public final void updatePlayer(Buffer buffer) {
		...
		skullIcon = buffer.get();
		prayerIcon = buffer.get();
		npcDefinition = null;
		teamId = 0;
		...
		if (model == null) {
			...
			for (int i_24_ = 0; i_24_ < 5; i_24_++) {
				if (appearanceColors[i_24_] != 0) {
					model.recolor(Game.anIntArrayArray1028[i_24_][0], Game.anIntArrayArray1028[i_24_][appearanceColors[i_24_]]);
					if (i_24_ == 1) {
	...
	public final Model getHeadModel() {
		...
		for (int index = 0; index < 5; index++) {
			if (appearanceColors[index] != 0) {
				headModel.recolor(Game.anIntArrayArray1028[index][0], Game.anIntArrayArray1028[index][appearanceColors[index]]);
				if (index == 1) {
