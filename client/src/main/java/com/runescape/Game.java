...
public class Game extends GameShell {
	...
	public final void sortGroundItems(int x, int y) {
		...
		if (groundItems == null) {
			...
			currentScene.method503((byte) 7, x, position, groundItemOne, method42(currentSceneId, y * 128 + 64, true, x * 128 + 64), groundItemTwo, item, currentSceneId, y);
		}
	}
	...
	public final void processWidgetClick(int i, Widget widget, int i_112_, int i_113_, int i_114_, int i_115_) {
		try {
			...
			Signlink.reportError("38948, " + i + ", " + widget + ", " + i_112_ + ", " + i_113_ + ", " + i_114_ + ", " + i_115_ + ", " + runtimeexception.toString());
			throw new RuntimeException();
		}
	}
	...
	public final Archive requestArchive(int archiveId, String archiveAlias, String archiveName, int archiveCRC, int loadPercentage) {
		try {
			...
			Signlink.reportError("44021, " + archiveId + ", " + archiveAlias + ", " + archiveName + ", " + archiveCRC + ", " + loadPercentage + ", " + runtimeexception.toString());
			throw new RuntimeException();
		}
	}
			...
			if (pressedRow >= 0) {
				...
				if (menuActionId == 516) {
					if (!actionMenuOpen) {
						...
						currentScene.method534(false, frameId - 4, itemSlotId - 4);
					}
				}
		...
		try {
			if (player != Game.localPlayer && menuActionRow < 400 && !bool) {
				...
				if (player.anInt1743 == 0) {
					string = player.playerName + Game.method110(Game.localPlayer.combatLevel, player.combatLevel, true) + " (level-" + player.combatLevel + ")";
				} else {
			...
			for ( /**/; animableobject != null; animableobject = (GameAnimableObject) aLinkedList1081.getPrevious()) {
				if (animableobject.plane != currentSceneId || animableobject.transformCompleted) {
					...
					if (animableobject.transformCompleted) {
						...
						currentScene.method507(animableobject.plane, 0, (byte) 6, animableobject.z, -1, animableobject.y, 60, animableobject.x, animableobject, false);
					}
				}
			}
		} catch (RuntimeException runtimeexception) {
	...
	private final void updatePlayerMask(int mask, int playerId, Buffer buffer, Player player) {
		do {
			try {
				...
				Signlink.reportError("50326, " + mask + ", " + playerId + ", " + buffer + ", " + player + ", " + runtimeexception.toString());
				throw new RuntimeException();
			}
			break;
		} while (false);
	...
	public final void method146() {
		try {
			...
			if (!aBoolean1185) {
				...
				method144(0, 600 + i * 3, i, anInt1039, method42(currentSceneId, Game.localPlayer.yWithBoundary, true, Game.localPlayer.xWithBoundary) - 50, i_1180_, anInt1040);
			}
			int i;
			if (!aBoolean1185) {
				i = method120();
