...
public class OndemandController extends Requester implements Runnable {
	...
	String[] versionFiles = { "model_version", "anim_version",
		"midi_version", "map_version" };
	...
	while (onDemandNode != null) {
	    ...
	    if (!verify(fileVersions[onDemandNode.type][onDemandNode.id], fileCrc[onDemandNode.type][onDemandNode.id], buffer)) {
		buffer = null;
	    }
	    synchronized (wanted) {
		if (buffer == null) {
