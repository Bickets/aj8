...
public final class PlayerSynchronizationMessageEncoder extends MessageEncoder<PlayerSynchronizationMessage> {
	...
	for (SynchronizationSegment segment : message.getSegments()) {
		...
		putAddCharacterUpdate((AddCharacterSegment) segment, message, builder);
	...
	playerProperties.put(DataType.BYTE, 0);// block.getSkullIcon());
	...
	builder.put(DataType.SHORT, DataOrder.LITTLE, DataTransformation.ADD, block.getMarkerX());
	builder.put(DataType.SHORT, DataTransformation.ADD, block.getMarkerY());
